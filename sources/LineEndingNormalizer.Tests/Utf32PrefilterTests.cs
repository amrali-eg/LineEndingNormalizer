...
///
/// Ported from EncodingChecker, which shares this detector. A prefilter optimisation, not
/// a correctness fix: invalid UTF-32 was already rejected downstream and still is, and
/// surrogates are unaffected by the added test.
/// </summary>
public sealed class Utf32PrefilterTests
{
    ...
    public void SupplementaryPlaneText_IsStillDetected(bool bigEndian, string expected)
    {
        ...
        // the case most at risk from an over-eager bound.
    ...
    {
        // The motivating case: ASCII-range UTF-16 has a zero in every other byte.
